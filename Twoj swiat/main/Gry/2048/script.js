document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateScore(value) {
        ...
        // Check if score reached 100 and redirect if appropriate
        if (currentScore >= 5000) {
            checkAndRedirect();
        }
    }

    // Function to check if a month has passed since the last redirect
    function checkAndRedirect() {
        ...
        if (!lastRedirectDate || new Date(lastRedirectDate).getTime() + oneMonth < currentDate.getTime()) {
            // Update last redirect date
            localStorage.setItem('lastRedirectDate', currentDate.toISOString());

            // Redirect to a different page (change 'your-page-url.html' to the actual URL)
            window.location.href = "../Konkurs/index.html"; // Replace with the actual URL
        }
    }
    ...
    function resetLocalStorage() {
        localStorage.removeItem('lastRedirectDate');
    }
    document.getElementById('reset-btn').addEventListener('click', () => {
        resetLocalStorage();
        // Opcjonalnie, możesz także zresetować inne elementy gry
        restartGame();
    });
